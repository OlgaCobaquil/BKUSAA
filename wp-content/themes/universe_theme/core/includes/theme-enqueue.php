...
function core_main_styles() {
	...
	if ($color) {
		wp_register_style('predefined-color', get_template_directory_uri(). '/core/css/colors/'.$color.'.css');
		wp_enqueue_style('predefined-color');
	} else {
		wp_register_style('default-color', get_template_directory_uri(). '/core/css/colors/blue.css');
		wp_enqueue_style('default-color');
		
	}
	
	// Custom CSS
	wp_register_style('custom_css', get_template_directory_uri() . '/core/css/custom.css.php', 'style');
