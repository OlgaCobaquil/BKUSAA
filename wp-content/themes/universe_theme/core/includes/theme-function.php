...
function core_print_map($lat, $lon) {
	$lat_info = $lat;
	$lon_info = $lon;
    $api_key = of_get_option('map_api_key');
	?>
	<script>
		function initialize() {
			var myLatlng = new google.maps.LatLng(<?php echo wp_json_encode( $lat_info );?>,  <?php echo wp_json_encode( $lon_info );?>);
			var mapOptions = {
				zoom : 8,
				center : myLatlng
			};
		...
		function loadScript() {
			...
			<?php if ( !empty( $api_key ) ) : ?>                 
                script.src = 'https://maps.googleapis.com/maps/api/js?key=<?php echo esc_attr( $api_key ); ?>&callback=initialize';
            <?php else: ?>              
